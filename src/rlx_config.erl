...
%% 搜索relx.config文件，然后将该文件中的信息解析出来
do(State) ->
...
search_for_dominating_config({ok, Cwd}) ->
    ...
    case ec_file:exists(ConfigFile) of
        true ->
            {ok, ConfigFile};
        false ->
            %% 如果Cwd目录没有寻找到对应的文件,则继续从Cwd目录的父目录继续寻找
...
search_for_dominating_config(State0) ->
    ...
    case search_for_dominating_config({ok, Cwd}) of
        {ok, Config} ->
            ...
            %% 将relx.config文件的父目录作为state结构里父目录
            State1 = rlx_state:root_dir(State0, RootDir),
            %% 设置relx.config文件的绝对路径,同时加载该配置文件的信息
...
load_config(ConfigFile, State) ->
    ...
    case Config0 of
        ...
        _ ->
            ...
            %% 将配置信息中的所有信息按照不同格式存储到rlx_state数据结构中
...
%% 将relx.config文件中的配置和默认的配置信息进行合并
